...
2. An internal NSIS package owns application files, the Start Menu shortcut, optional TextRecast Desktop/Startup shortcuts, and Windows uninstall registration. It is embedded in the setup host and is not published separately.
...
The completion page keeps Launch TextRecast, Add a desktop shortcut, and Start TextRecast when I sign in as explicit checkboxes. All three default to enabled. Finish copies the package-owned Start Menu shortcut to the current user's Desktop and Startup folders according to those choices. Noninteractive installation applies the same desktop and sign-in defaults.
...
The result is written to `artifacts/release/TextRecast-v0.3.0-win-x64-setup.exe` with a matching SHA-256 file.
...
- The Start Menu shortcut and uninstall metadata are written only after payload activation. Completion can copy that owned shortcut to the current user's Desktop and Startup folders. Uninstall removes the product-owned installation tree, all three known TextRecast shortcuts, and registry entries.
